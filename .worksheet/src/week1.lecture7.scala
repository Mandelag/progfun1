 ...
 */
object lecture7 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(302); 
  def oldFactorial(x: Int): Int = {
  	if (x <= 1) 1 else x*factorial(x-1)
  };System.out.println("""oldFactorial: (x: Int)Int""");$skip(122); 
  
	def tailFactorial(x: Int, aggregate:Int=1): Int = {
  	if (x <= 1) aggregate else tailFactorial(x-1, x*aggregate)
  };System.out.println("""tailFactorial: (x: Int, aggregate: Int)Int""");$skip(189); 
